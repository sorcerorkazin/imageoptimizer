...
$selectStatement = "SELECT imageID, originalImageURL, optimizedImageURL, originalSize, newSize FROM optimizedImages WHERE pageNumber = ? AND storeHash = ? AND isSent = 1";
...
while (@row = $statement->fetchrow_array) {
    ...
    $response .= '    <div><img src=\"'.$row[1].'\"></div>';
    ...
    $response .= '</div>';
    $response .= '<div class=\"preview\">';
    $response .= '    <div><img src=\"'.$row[2].'\"></div>';
